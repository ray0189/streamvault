...
Instead of piping directly into bash, use:
...
StreamVault supports Windows + WSL2.

If running StreamVault inside Ubuntu on WSL and Stremio on Windows, use:

```text
http://127.0.0.1:7000/config/YOUR_CONFIG_ID/manifest.json
```

instead of the temporary WSL `172.x.x.x` address.

WSL IPs can change between sessions, while `127.0.0.1` is usually stable.

Example:

```text
http://127.0.0.1:7000/config/986c9e83-c16f-4685-8b1b-7ce1eb4cbc11/manifest.json
```
