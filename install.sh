...
#
# This installer keeps StreamVault simple: config is stored in .env, including
# TORBOX_API_KEY and ADMIN_PASSWORD. Do not commit .env.
...
# StreamVault v2 — Environment
# Run 'streamvault setup' or 'sudo bash install.sh --reconfigure' to configure interactively.
...
# Dashboard/CLI admin password
ADMIN_PASSWORD=

# Signing key for stream URLs — generated by install.sh if blank
SECRET_KEY=

# Public URL — set to your trusted HTTPS reverse-proxy URL when using Stremio iOS.
# Example: https://your-hostname.example:8444
PUBLIC_BASE_URL=
...
# Cloudflare (optional — set by setup wizard if used)
CF_TUNNEL_TOKEN=
CF_DOMAIN=
ENVEOF
}
...
maybe_prefer_ipv4_for_node() {
  # Some VPS images advertise IPv6 DNS records but have no working IPv6 route.
  # Node/npm may then try IPv6 first and fail with ENETUNREACH even though apt,
  # git and IPv4 curl work. Detect that exact shape and prefer IPv4 for Node.
...
warn_if_https_needs_proxy() {
  local base=""
  base="$(grep '^PUBLIC_BASE_URL=' "$INSTALL_DIR/.env" 2>/dev/null | cut -d= -f2- || true)"
  case "$base" in
    https://*)
      warn "PUBLIC_BASE_URL is HTTPS. StreamVault itself serves HTTP only; keep Caddy/Nginx proxying HTTPS to http://127.0.0.1:${APP_PORT:-$PORT}."
      ;;
  esac
}
